...
export const EvidencePanel: React.FC<EvidencePanelProps> = ({ incidentId }) => {
  ...
  const supportingItems = evidenceList.filter(
    (ev) => ev.type !== 'MITIGATING'
  );
  const mitigatingItems = evidenceList.filter(
    (ev) => ev.type === 'MITIGATING'
  );
    ...
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-sm space-y-4">
      ...
      <div className="flex items-center justify-between border-b border-slate-800 pb-3">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base font-semibold text-slate-100">Evidence Panel</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2 text-xs font-semibold text-red-400 font-mono uppercase tracking-wider">
              ...
              {supportingItems.map((item) => {
                ...
                return (
                  <div
                    ...
                    className={`p-3 bg-slate-950/70 border rounded-lg hover:border-amber-500/40 transition-all cursor-pointer space-y-1.5 ${
                      isSelected ? 'border-amber-500/60 bg-slate-900' : 'border-slate-800'
                    }`}
                  >
                    <div className="flex items-center justify-between text-xs">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-mono text-[10px] px-1.5 py-0.2 rounded bg-amber-500/10 text-amber-400 border border-amber-500/30">
                          {item.type}
                        </span>
                      </div>
                      <ChevronRight
                      ...
                      <div className="pt-2 border-t border-slate-800 space-y-1.5 text-[11px] text-slate-400 font-mono">
                        {item.event_id && (
                          <div className="flex items-center justify-between">
                            ...
                            <span className="text-blue-400 bg-slate-900 px-1.5 py-0.5 rounded border border-slate-800">
                              {item.event_id}
                            </span>
                          </div>
                        )}
              ...
              {mitigatingItems.map((item) => {
                ...
                return (
                  <div
                    key={item.evidence_id}
                    onClick={() => setSelectedEvidence(isSelected ? null : item)}
                    className={`p-3 bg-slate-950/70 border rounded-lg hover:border-emerald-500/40 transition-all cursor-pointer space-y-1.5 ${
                      isSelected ? 'border-emerald-500/60 bg-slate-900' : 'border-slate-800'
                    }`}
                  >
                    <div className="flex items-center justify-between text-xs">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-mono text-[10px] px-1.5 py-0.2 rounded bg-emerald-500/10 text-emerald-400 border border-emerald-500/30">
                          {item.type}
                        </span>
                      </div>
                      <ChevronRight
